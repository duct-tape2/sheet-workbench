...
Hardened public deployment, live Google OAuth/Picker/writeback, native PostgreSQL/Docker, mail delivery, hosted monitoring, and native Excel round-trip remain release gates. Exact trusted-origin configuration is implemented but needs proxy-level verification. Dataset-delete and account-delete routes are not implemented. See [docs/INTEGRATION_NOTES.md](docs/INTEGRATION_NOTES.md).
