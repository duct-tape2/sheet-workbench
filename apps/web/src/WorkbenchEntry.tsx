...
export default function WorkbenchEntry() {
  ...
  return <Suspense fallback={<p role="status">{locale === 'ko' ? '작업 화면을 여는 중…' : 'Opening your workbench…'}</p>}>
    {mode === 'team' || mode === 'demo' ? <><TeamApp /><div style={{ padding: 'var(--space-sm)' }}><button onClick={() => go('local')}>{locale === 'ko' ? '개인 파일 도구' : 'Personal file tools'}</button></div></> :
      ...
      }} onLegacy={() => go(STATIC_DEMO ? 'demo' : 'team')} />}
  </Suspense>;
}
